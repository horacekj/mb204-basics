...
\begin{document}
\title{\vspace{-1.5cm}Základy teorie čísel}
...
%		$\mu(1) = 1$.
	...
	\item \textbf{Malá Fermatova věta}\\
		...
		Napříkad pro \textit{Carmichaelova čísla} věta platí, ale přesto tato
	...
	\begin{enumerate}
		...
		\item \textbf{Gaussovo lemma}\\
			...
			$\forall x \in S : a \cdot x \equiv \pm s \in S$. $\mu(a)$ je
			počet záporných prvků $s$ v~předchozím výrazu.

	\end{enumerate}

	\item \textbf{Testy složenosti}\\
		Pokud je číslo $x$ složené, test na složenost to s jistotou pozná, pokud
		však $x$ není složené (je to prvočíslo), z~výsledku testu nelze činit
		závěry.

		Testy testují některou z~\textit{nutných podmínek prvočíselnosti}:
		\textit{je-li $x$ prvočíslo, pak platí věta $v$} $\Leftrightarrow$
		\textit{neplatí-li věta $v$, $x$ je složené}. Pokud $v$ platí,
		o~složenosti nic dalšího nevíme.

		\begin{enumerate}
			\item \textbf{Fermatův test}\\
				Existuje-li pro dané $N$ nějaké $a \not \equiv 0 \pmod{N}$
				takové, že $a^{N-1} \not \equiv 1 \pmod{N}$, pak $N$ je
				složené.

				Bohužel, hledání $a$ je mnohdy náročné, takže i~odhalení
				samotné složenosti může být náročné (viz
				\textit{Carmichaelova čísla}).

			\item \textbf{Eulerův test}\\
				Je-li $N$ prvočíslo, $a \in \mathbb{Z}$ a $N \nmid a$, pak
				\[ a^{\frac{N-1}{2}} \equiv \genfrac(){}{1}{a}{N} \pmod{N}. \]
		\end{enumerate}

	\item \textbf{Pseudoprvočísla} jsou taková čísla, která projdou testem na
		složenost a nejsou jim odhalena jako složená.

	\item \textbf{Mersenneho čísla} jsou čísla tvaru $M_n = 2^n -1$.
	\item \textbf{Fermatova čísla} jsou čísla tvaru $F_n = 2^{2^n} + 1$.
